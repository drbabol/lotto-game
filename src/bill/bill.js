...
class Bill {
	constructor(id, numbers, typeAndBet, city) {
		...
		this.totalBet = this.calculateTotalBet(); //total bet of the bill
	}
	/**
	...
	/**
	 * function to calculate the total bet of the curren bill
	 * @returns {Number}
	 */
	calculateTotalBet() {
		const objTypeBet = this.typeAndBet;
		const totalBet = Object.values(objTypeBet).reduce((accumulator, currValue) => accumulator + currValue, 0);
		return totalBet;
	}
		...
		for (let i = 0; i < this.city.length; i += 3) {
			group3Cities.push(
				`│${this.city
					.slice(i, i + 3)
					.join(', ')
					.padStart(this.city.slice(i, i + 3).join(', ').length + 1, ' ')
					.padEnd(maxWidthPrint - 1, ' ')}│`
			);
		}
		const cityString = group3Cities.join(' \n');

		const totalBet = this.totalBet; //Object.values(this.typeAndBet).reduce((accumulator, currentValue) => accumulator + currentValue, 0);
	 ...
	 * {id: {Venezia:{numbers:[1,2,3,4],types:['Ambata','Quaterna'],prize: 120000},Bari:{numbers:[1,3],types:['Ambata'],prize: 22.46}}}
	 */
		...
		this.city.forEach((city) => {
			...
			for (const bet in this.typeAndBet) {
				...
				if (commonNumPerCity.length >= thisTypeConverted) {
					...
					winningCityNum[city] = { id: this.id, numbers: commonNumPerCity, types: winningBet, prize: Math.round(prize.reduce((acc, curr) => acc + curr, 0)) };
				}
			}
		});
	 ...
	 */
	static printResult(winnBill, id) {
		let width = 36;
		if (winnBill === false) {
			const output = `${'┌'.padEnd(width, '─')}┐\n${`│ Bill #${id} did not win... ✘`.padEnd(width, ' ')}│\n${'└'.padEnd(width, '─')}┘`; //this.id
			return output;
		} else {
			const citisPrint = [];
			for (const city in winnBill) {
				if (winnBill[city]['types'].join(', ').length + 12 > 36) {
					width = winnBill[city]['types'].join(', ').length + 14;
				}

				const emptyLine = `${'│'.padEnd(width, ' ')}│`;
				citisPrint.push(
					`${`│ ${city} WIN with:  ✔`.padEnd(width, ' ')}│\n${`${`│ Types`.padEnd(10, ' ')}→  ${winnBill[city]['types'].join(', ')}`.padEnd(width, ' ')}│\n${`│ Numbers →  ${winnBill[city][
						'numbers'
			...
			if (totalPrize === totalNetPrize) {
				const output = `${'┌'.padEnd(width, '─')}┐\n${`│ BILL #${id.toString()} WIN!`.padEnd(width, ' ')}│\n${'╞'.padEnd(width, '═')}╡\n${citisPrint.join(
					`\n`
